...
If miners outside the Gupax instance are mining on P2Pool for the same address, Gupax will send too much on P2Pool because it doesn't take extra HR outside his control into account.
...
If miners outside the Gupax instance are mining on XvB for the same address, Gupax will maybe send too much (more than enough for the round) or too less (could have been in better round) on XvB.
...
Gupax process will check frequently the output of the P2Pool command "status" without impacting the console output of the P2Pool tab to get the current shares.
### know oHR on P2Pool
Gupax watch for sidechain HR for his address on the output of status command of P2Pool in a way that do not disturb the output of console in the UI.
### know oHR on XvB
Gupax receive this data from XvB API.
### Knowing the HR generated by Gupax
Gupax will simply watch the values retrieved by XMRig process.
