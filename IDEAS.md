...
Github is proprietary. If Gupax aims to be free software, it should not only be available on this proprietary platform and we should explore other options to be github free.
We can use [Radicle](https://radicle.xyz/) to get Gupax on a p2p collaboration stack.
...
To get rid of integrating nodes list, we can include a crawler like monero.fail inside Gupax.
### Integrate a Monero Node
[Done](https://github.com/Cyrix126/releases/tag/v1.5.0)  
If we want Gupax to help user mine in the most decentralized way, we should offer them to run a monero node.
...
Currently in the [README](https://github.com/Cyrix126/gupax#what-is-gupaxmrvsbeast), there is no explanation on what XMRvsBeast is.
The README explains what XMRvsBeast does but not what it is.
we could improve this with either creating another chapter or expanding the [current one](https://github.com/Cyrix126/gupax#what-is-gupaxmrvsbeast).
### Generated wallet
If Gupax could create a wallet and put the primary address in P2Pool tab automatically, it would remove a manual step for the user.
...
If Gupax could register the user automatically to the raffle, it would remove a manual step for the user.
...
It could be an option to ask at first start.
### Setup Guide
At first start, a guide could ask the user what it intends to do with Gupax (create node, create wallet, use XMRig-Proxy, participate in raffle...) and do the setup for him and show him what it must do manually. An option to skip this guide would be present for advanced users.
### Very noob mode
...
[Done](https://github.com/Cyrix126/gupax/commit/075beddea19b3f09e1e7b2327e235814fe588520)  
...
### Better defaults
[Done](https://github.com/Cyrix126/gupax/commit/6cb767a342bec2df3358b10826a1ec1dee57fc76) and [Done](https://github.com/Cyrix126/gupax/commit/a102bdbee2e4c0bc8785f9e638d3e54958d79489)  
Reduce in/out peers, remote nodes by default
#### Set fixed font size, do not resize with size of window
[Done](https://github.com/Cyrix126/gupax/releases/tag/v1.6.0)  
...
[Done](https://github.com/Cyrix126/gupax/releases/tag/v1.7.0)  
...
## Making Gupax Support more environments
### Packaging
Add repository/AUR for Gupax and a status of packaging distro/version on the README.
...
### Add more target
Gupax could add support for Linux ARM64 since both P2Pool and XMRig can compile on this target.
### CLI for Algorithm
...
### Web UI
To be able to control and watch Gupax from another device, a daemon mode could be built with a web UI front-end. This web UI can also be a web app to be used on mobile, and send push notifications like the desktop notifications.
Gupax on desktop can provide a qr code to give the address and authentication data.
### Refactor size of text
[Done](https://github.com/Cyrix126/gupax/releases/tag/v1.6.0)  
Gupax currently resize texts/widgets based on the window size. Instead, the text/widget size should be decided by the OS/config, scroll bar should be used when there is not enough space. It will allow to use Gupax on different ratio of screen.
...
On the XMRig tab, inform users about manual optimizations that Gupax can't control. For example, disabling hyper-threading in BIOS is recommended.
...
### Systray icon
Enable a way to put Gupax in background, managing it with a systray icon.
### Auto-Launch
Option to launch Gupax at startup
### API of Algorithm
To make the Algorithm controllable outside of Gupax.
### Graphs history of HR
The user could see how the HR was given on P2Pool and XvB.
### Update XMRig benchmark from Gupax
...
Updates can be applied only when Gupax is restarted. Make a button to auto-restart after updates.
...
A lot of tests since the fork makes less sense and a lot of new situation needs to be tested. Time should be taken to add new tests and make Gupax more robust.
egui_kittest library can be used to test the interaction of the UI directly. 
### Manually set HR for XvB algo
Done by [Sina](https://github.com/mostafaei2002) [PR](https://github.com/Cyrix126/gupax/pull/11)  
...
[Done](https://github.com/Cyrix126/gupax/releases/tag/v1.2.0)  
...
Gupax could offer this XMRig-instance and control it like it was a normal XMRig instance.
...
The algorithm would still check the estimation made by the P2Pool instance of Gupax and warn the user if it seems there is too much difference between the data of the stratum server and the one of P2Pool. It could prevent the user to forget to configure a miner to the stratum P2Pool.
Could also be an option in advanced tab of XvB warning the user that he should point all his miners to the P2Pool instance of Gupax to take them into account.
...
### Check signature of updates with Gupax
let the build in updater of Gupax check the signature of the release to confirm that the releases has been signed by the right key.
